...
SURFACE_OUT_DIR="../dataset/$dataset_type/01-benchmark_pdbs_surfaces"
...
while IFS= read -r LINE; do
    ...
    PLY1="${SURFACE_OUT_DIR}/res_${RES}/${NAME1}.ply"
    PLY2="${SURFACE_OUT_DIR}/res_${RES}/${NAME2}.ply"
    ...
    if [ -f "$CHAIN_PDB1" ] && [ -f "$PLY1" ]; then
        echo "[INFO] Chain 1 already done (PDB + PLY exist), skipping: $NAME1"
    else
        python compute_protein_surface.py "$PDB_PATH" "$NAME1" "$RES" "$PDB_OUT_DIR" "$SURFACE_OUT_DIR"
    fi

    # -------- Chain 2 --------
    if [ -f "$CHAIN_PDB2" ] && [ -f "$PLY2" ]; then
        echo "[INFO] Chain 2 already done (PDB + PLY exist), skipping: $NAME2"
    else
        python compute_protein_surface.py "$PDB_PATH" "$NAME2" "$RES" "$PDB_OUT_DIR" "$SURFACE_OUT_DIR"
    fi
