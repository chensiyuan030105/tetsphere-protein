...
class TetMeshGeometry(torch.nn.Module):
    ...
    def compute_vertex_normal(self):
        ...
        forward_data = TetMeshGeometryForwardData(
            self.tet_v, self.tet_elem, self.surface_vid, self.surface_fid, smooth_barrier_energy)
        return forward_data._compute_vertex_normal()
    ...
    def __init__(self, cfg: Config):
        ...
        if self.cfg.initial_mesh_path != "":
            ...
            if not self.cfg.load_precomputed_tetwild_mesh:
                ...
                # shift vertices so the geometric center is at the origin
                vertices = template_sphere.vertices
                center = np.mean(vertices, axis=0)
                vertices = vertices - center
                scale = np.max(np.linalg.norm(vertices, axis=1))
                vertices = vertices / scale
                template_sphere.vertices = vertices                

                # verify
                sphere_center = np.mean(template_sphere.vertices, axis=0)
                print("Sphere center after centering:", sphere_center)  # should be [0, 0, 0]
                ...
                for sp_i in range(num_spheres):
                    ...
                    print(f"Sphere {sp_i} vertices (after scaling and translation):")
                    print(f"Sphere {sp_i} center (approx): {np.mean(sphere_v, axis=0)}")
                    ...
                    cmd = f"{tetwild_exe} --input {self.cfg.tetwild_cache_folder}/temp{sp_i}.obj --output {self.cfg.tetwild_cache_folder}/temp{sp_i}.msh --targeted-num-v {tri_v.shape[0] * self.cfg.tetwild_scale} --epsilon 0.001 --is-quiet"
